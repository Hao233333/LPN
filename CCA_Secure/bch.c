...
//clang -o bch bch.c -I/opt/homebrew/opt/openssl@3/include -L/opt/homebrew/opt/openssl@3/lib -lssl -lcrypto

//./bch
// /Users/qh/Documents/projects/LPN/LPN/BCH
#include <math.h>
#include <stdio.h>
#include <stdlib.h>
#include <time.h>
...
unsigned char* encode_bch(unsigned char *data)//参数修改
/*
...
//    printf("\n");
    ...
    // 将 recd 中的值赋给 encodedMessage
    for (i = 0; i < length; i++) {
        encodedMessage[i] = recd[i];
    }
...
//    printf("\n");
    ...
    unsigned char* decodedMessage = (unsigned char*) malloc(sizeof(unsigned char) * k);
    if(decodedMessage == NULL) {
        fprintf(stderr, "Failed to allocate memory for the decoded message.\n");
        exit(1);
    }
    //========================================
    if (syn_error) {    /* if there are errors, try to correct them */
        ...
        if (l[u] <= t) {/* Can correct errors */
            ...
            printf("sigma(x) = (represents the error location polynomial, which can locate and correct the error bits in the BCH code by calculating and searching the root./代表错误位置多项式，通过计算和搜索根，可以定位并纠正BCH码中的错误位。)\n");
            
            for (i = 0; i <= l[u]; i++)
                printf("%3d ", elp[u][i]);
            printf("\n");
            printf("Roots: (error locations/错误位置)");
            ...
            *errorCount = count;
            ...
            //                        return NULL;
            //                    }//daozhe
            //将解码的结果存入decodedMessage，并返回
            
            //            for (i = 0; i < k; i++) {
            //                decodedMessage[i] = recd[length - k + i];
            //            }
...
//        printf("%1d", recd[i]);
//        if ((i-length+k) && ((((i-length+k)+1) % 8) == 0))
//            printf(" ");
//    }
//    printf("\n");
//
